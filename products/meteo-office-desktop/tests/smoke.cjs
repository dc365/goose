...
assert.ok(mainSource.includes("require('./capabilities/remote-artifact-downloader.cjs')"));
assert.ok(mainSource.includes("require('./capabilities/streaming-output-guard.cjs')"));
assert.ok(mainSource.includes('void this.ensureRemoteOfficeArtifacts(request, sessionId)'));
assert.ok(mainSource.includes('this.captureRemoteOfficeArtifactUrls(notification.sessionId'));
assert.ok(mainSource.includes('const artifactSources = this.sessionRemoteArtifactUrlMap.get(sessionId)'));
assert.ok(mainSource.includes('const guardedText = this.handleAssistantOutput(notification.sessionId, text)'));
assert.ok(mainSource.includes('await this.ensureRemoteOfficeArtifacts(request, sessionId)'));
assert.ok(rendererSource.includes("artifact.metadata?.delivery === 'remote-download' ? '已保存到本机'"));
...
assert.ok(packageMacScript.includes('npm run goose:build'));
assert.ok(packageMacScript.includes("--extra-resource='../../target/release/goose'"));
...
const workspaceGooseCandidate = mainSource.indexOf("path.join(process.resourcesPath, binaryName)");
const sdkGooseCandidate = mainSource.indexOf("await import('@aaif/goose-sdk/node')");
assert.ok(workspaceGooseCandidate >= 0);
assert.ok(workspaceGooseCandidate < sdkGooseCandidate);
assert.ok(packageJson.scripts['goose:build'].includes("cargo build --release -p goose-cli --bin goose"));
