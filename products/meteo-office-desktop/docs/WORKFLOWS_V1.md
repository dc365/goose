...
- Workflow 定义这些能力按什么依赖、分支、审批和交付契约协作；
- Automation 负责何时、因为什么事件触发一个任务或工作流。

专家团仍然是面向用户的协作角色，工作流则是可被专家、专家团、项目和自动化复用的执行契约。两者可以互相组合，但不互相替代。
...
- Input、Trigger、Expert、LLM、Classifier、Extractor、Knowledge、Document、Transform、Assign、Tool、HTTP、Code、Workflow、Condition、Iteration、Join、Approval、Template、Delay、Output 二十一类通用节点；
...
- 个人专家绑定必需或建议工作流；
- 定时任务固定一个已发布工作流版本；
- 旧专家团和旧定时任务转换为工作流定义的适配器；
- 任务运行时递归解析子工作流、节点 Skill、专家依赖、Connector 和具体 Tool，并把已授权工作流作为版本固定的编排契约加载；
...
选中节点时，步骤或画布、右侧检查器和运行轨迹始终指向同一个节点。运行路径通过节点状态和连线颜色表达，不依赖复杂动画。
...
底部运行条展示最近一次状态，并提供结构试跑入口。运行详情使用画布、时间线、节点输出和审批区组合，避免把调试信息塞入对话正文。
...
解析工作流能力时会递归计算依赖闭包。任何必需的子工作流、Skill、专家、Connector 或具体 Tool 不存在、未连接或未被当前自定义授权选中，任务都会在启动前失败并指出缺失项。子工作流必须引用一个已发布的精确版本，循环引用和超深嵌套无法发布。
...
| 专家团 | 否 | 继续使用原有多 Agent DAG；提供 `toWorkflowDefinition` 适配器 | 只有需要跨专家团复用、审批、工具节点或可视化运行时再转为工作流 |
| 定时任务 | 否 | 继续创建普通任务；可选 `workflowRef` 固定一个发布版本 | 新自动化优先绑定工作流，Trigger 只负责触发 |
...
```text
harness/workflow.js                 工作流规范化、校验、发布、结构运行和兼容适配
...
1. 原生 Workflow Runtime，按节点创建独立 Agent 会话并并行调度；
